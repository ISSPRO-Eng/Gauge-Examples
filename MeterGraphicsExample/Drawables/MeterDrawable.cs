...
public class MeterDrawable : BaseDrawable, IDrawable
{
    ...
    public bool HorizontalMode { get; set; } = true;
    ...
    private void AddTickMarks(ICanvas canvas, int steps, Rect meter)
    {
        for (int i = 0; i < steps; i++)
        {
            ...
            if (i != 0)
            {
                ...
                if (ShowSeries)
                {
                    // Define the font and size explicitly
                    var fontSize = 12;  // Replace with your desired font size
                    ...
                    var text = percentOfMax.ToString();
